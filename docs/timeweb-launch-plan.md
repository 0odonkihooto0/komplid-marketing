...
**1.3. Почта на домене — ✅ 21.08.2026.** `hello@komplid.ru` создан. Печатается
в шапке (шире 1200px), мобильном меню, подвале, контактах, политике и
Schema.org — везде из `company.email`. Проверить, что MX, SPF, DKIM и DMARC
разошлись, после этапа 3. Сайт почту не отправляет — ящик для входящих.
...
**Готово, когда:** ✅ бакет создан, приватный, ключи выписаны, сквозная проверка
пройдена.
