...
// почти идентичных компонентах — здесь она вынесена один раз, роли передают данные.
...
export interface RolePricingProps {
  eyebrow: string;
  title: string;
  description: string;
...
export function RolePricing({ eyebrow, title, description, tiers }: RolePricingProps) {
    ...
    >
      <div className="wrap">
        <div className="section-head" style={{ textAlign: 'center', margin: '0 auto 32px' }}>
          ...
          <p style={{ marginLeft: 'auto', marginRight: 'auto' }}>{description}</p>
        </div>
        ...
        {/* Карточки тарифов */}
        <div className="role-pricing-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16 }}>
          {tiers.map((tier) => (
            <div
              key={tier.name}
              style={{
      ...
      <style>{`
        @media (max-width: 900px) {
          .role-pricing-grid { grid-template-columns: 1fr !important; }
        }
      `}</style>
    </section>
  );
